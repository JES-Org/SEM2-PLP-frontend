...
import {
	AssessmentAnswer as AnswerOptionType,
	AssessmentQuestion,
} from '@/types/assessment/assessment.type'
import { CheckCircle, MessageSquareText, Star, XCircle } from 'lucide-react' // Added Star for score display
...
const TakeAssessment = () => {
	...
	const [submissionGradedDetails, setSubmissionGradedDetails] = useState<
	...
	const totalPossibleAssessmentWeight = useMemo(() => {
		...
		return fetchedQuestions.reduce(
	...
	useEffect(() => {
		...
		if (submissionDetails?.id) {
			...
			if (
				...
				Object.keys(submissionDetails.answers).length > 0
			) {
		...
		for (const question of fetchedQuestions) {
			if (!answers[question.id] || answers[question.id].trim() === '') {
				const questionNumber =
					fetchedQuestions.findIndex((q) => q.id === question.id) + 1
		...
		return (
			<div className='ml-72 mr-10 mt-10 h-screen flex justify-center items-center'>
				...
				<p className='text-xl text-red-500'>
					...
					{JSON.stringify(error) || 'Assessment data not found.'}{' '}
				</p>{' '}
			</div>
		)
	}
	...
	const allQuestionsAnsweredForSubmission =
		answeredQuestionsCount === fetchedQuestions.length
				...
				<div className='w-full p-4 md:p-8 overflow-auto'>
					...
					{!isReviewMode && !hasSubmitted && fetchedQuestions.length > 0 && (
						<div className='mb-8'>
							<div className='flex justify-between mb-1 text-sm font-medium text-gray-700 dark:text-gray-300'>
								<span>Progress</span>
								<span>
									{answeredQuestionsCount} / {fetchedQuestions.length} Answered
								</span>
							</div>
							<div className='w-full bg-gray-200 rounded-full h-2.5 dark:bg-gray-700'>
								<div
									...
									style={{
										width: `${fetchedQuestions.length > 0 ? (answeredQuestionsCount / fetchedQuestions.length) * 100 : 0}%`,
									}}
								></div>
							</div>
						</div>
					...
					{isReviewMode && (
						<div className='mb-6 p-3 bg-yellow-100 border border-yellow-300 text-yellow-700 dark:bg-yellow-900 dark:text-yellow-300 dark:border-yellow-700 rounded-md text-center text-sm'>
							You are in review mode. Your submitted answers and scores are
							shown below.
						</div>
					)}
					{fetchedQuestions.length > 0 ? (
						<div className='space-y-8'>
							{fetchedQuestions.map((question, questionIndex) => {
								...
								const manuallyGradedScore =
									isReviewMode && submissionGradedDetails
										? submissionGradedDetails[question.id]
										: undefined

								return (
									<div
										key={question.id}
										id={`question-wrapper-${questionIndex}`}
										className='p-4 sm:p-6 bg-white dark:bg-gray-800 shadow-md rounded-lg border border-gray-200 dark:border-gray-700'
									>
										<div className='flex flex-col sm:flex-row justify-between items-start mb-3'>
											...
											<Badge
												variant='outline'
												className='mt-1 sm:mt-0 ml-0 sm:ml-2 whitespace-nowrap'
											>
										...
										{question.question_type === 'multiple_choice' ? (
											...
											>
												{question.answers.map(
													(answerOption: AnswerOptionType) => {
														let optionStyle =
															'bg-gray-50 border-gray-300 hover:bg-gray-100 dark:bg-gray-750 dark:border-gray-600 dark:hover:bg-gray-700'
														let feedbackIcon = null

														if (isReviewMode && studentResponse !== undefined) {
															const isSelectedByStudent =
																String(answerOption.id) === studentResponse
															const isCorrectOption = answerOption.is_correct

															if (isSelectedByStudent) {
																if (isCorrectOption) {
																	optionStyle =
																		'bg-green-100 border-green-500 ring-2 ring-green-400 dark:bg-green-700 dark:border-green-500'
																	feedbackIcon = (
																		<CheckCircle className='h-5 w-5 text-green-700 dark:text-green-300 ml-auto' />
																	)
																} else {
																	optionStyle =
																		'bg-red-100 border-red-500 ring-2 ring-red-400 dark:bg-red-700 dark:border-red-500'
																	feedbackIcon = (
																		<XCircle className='h-5 w-5 text-red-700 dark:text-red-300 ml-auto' />
																	)
																}
															} else if (isCorrectOption) {
																optionStyle =
																	'bg-green-50 border-green-400 dark:bg-green-800 dark:border-green-600'
																feedbackIcon = (
																	<CheckCircle className='h-5 w-5 text-green-600 dark:text-green-400 ml-auto opacity-80' />
																)
															}
														} else if (
															studentResponse === String(answerOption.id)
														) {
															optionStyle =
																'bg-blue-100 border-blue-500 dark:bg-blue-700 dark:border-blue-500 shadow-md'
														}
											...
											<div className='mt-3 space-y-3'>
												...
												{isReviewMode &&
													manuallyGradedScore !== undefined &&
													manuallyGradedScore !== null && (
														<div className='flex items-center mt-2 p-2 bg-indigo-50 dark:bg-indigo-900 border border-indigo-200 dark:border-indigo-700 rounded-md'>
															...
															<p className='text-sm font-semibold text-indigo-700 dark:text-indigo-300'>
																Score Awarded: {manuallyGradedScore} /{' '}
																{question.weight || 0}
															</p>
														</div>
													)}
							...
							<div className='text-center pt-6'>
								{hasSubmitted ? (
									...
									<Button
										...
										disabled={
											!allQuestionsAnsweredForSubmission ||
											isSubmitting ||
											isLoading
										}
									>
						...
						<div className='bg-yellow-100 text-yellow-800 border border-yellow-300 px-4 py-3 rounded-md text-center shadow-sm'>
								🚫 No Question available yet for this assessment.
								<Button
										size='lg'
										onClick={() => router.back()}
										variant='outline'
